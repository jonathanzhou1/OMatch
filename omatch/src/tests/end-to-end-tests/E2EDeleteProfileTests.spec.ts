...
test("E2E, integration: check error upon incorrect password", async ({
  page,
}) => {
  // Create an account
  await createAccountTestHelper(
  ...
  await password.fill("joemungus burger sus");
  await submitPasswordButton.click();

  //see error message
  await expect(
    page.getByText(
      "Invalid password given for the account associated with email: giannis@gmail.com.",
      { exact: true }
    )
  ).toBeVisible();

  //Fill in correct password now, and delete
  await password.fill("1234567");
  await submitPasswordButton.click();
...
test("E2E, integration: cancel confirmation, then delete profile", async ({
  page,
}) => {
  // Create an account
  await createAccountTestHelper(
  ...
  // Deny Intent to Delete
  const denyButton = page.getByRole("button", { name: "No" });
